...
const RESOURCE = "carbon";
const MINING_MINUTES = 15;
...
const run = async () => {
  ...
  if (!fleetAccount.state.Idle) {
    throw Error("fleet is expected to be idle before mining");
  }
  ...
  if (!rx.value.isOk()) {
    throw Error("fleet failed to start mining");
  }
  ...
  await new Promise((resolve) =>
    setTimeout(resolve, MINING_MINUTES * 60 * 1000)
  );
  ...
  // Check that the transaction was a success, if not abort
  if (!rx.value.isOk()) {
    throw Error("fleet failed to deposit mined resources");
  }
