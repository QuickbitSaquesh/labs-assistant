...
const handlerFleetMovementWarp = async (cmds: FleetMovementWarp[]) => {
  ...
  for (let i = 0; i < cmds.length; i++) {
    ...
    const ix = await sageFleetHandler.ixWarpToCoordinate(
      fleetPubkey,
...
const handlerStarbaseHangarSupplyCargo = async (
  cmds: StarbaseHangarSupplyCargo[]
) => {
  const ixs = [];
  ...
  for (let i = 0; i < cmds.length; i++) {
    ...
    for (let j = 0; j < cmd.cargo.length; j++) {
      ...
      const ix = await sageFleetHandler.ixDepositCargoToFleet(
        fleetPubkey,
        cargoPodToKey,
        tokenMint,
        amount
...
const handlerStarbaseHangarUnloadCargo = async (
  cmds: StarbaseHangarUnloadCargo[]
) => {
  const ixs = [];
  ...
  for (let i = 0; i < cmds.length; i++) {
    ...
    for (let j = 0; j < cmd.cargo.length; j++) {
      ...
      const ix = await sageFleetHandler.ixWithdrawCargoFromFleet(
        fleetPubkey,
        tokenMint,
        amount
      );
      ...
      case path === "/fleet":
        ...
        if (fleetName) {
          ...
          const getPlayerProfileFleets = async () => {
            const fleets = await sageGameHandler.loadPlayerProfileFleets(
              playerProfilePubkey
            );
          ...
          const parseSageCmd = async () => {
            ...
            switch (true) {
              case jsonCmd.FleetMiningOpsStart !== undefined:
                ixs = await handlerFleetMiningOpsStart(
                  jsonCmd.FleetMiningOpsStart
                );
                break;
              case jsonCmd.FleetMiningOpsStop !== undefined:
                ixs = await handlerFleetMiningOpsStop(
                  jsonCmd.FleetMiningOpsStop
                );
                break;
              ...
              case jsonCmd.FleetMovementExitWarp !== undefined:
                ixs = await handlerFleetMovementExitWarp(
                  jsonCmd.FleetMovementExitWarp
                );
                break;
              ...
              case jsonCmd.StarbaseUndockFleet !== undefined:
                ixs = await handlerStarbaseUndockFleet(
                  jsonCmd.StarbaseUndockFleet
                );
                break;
              case jsonCmd.StarbaseHangarSupplyCargo !== undefined:
                ixs = await handlerStarbaseHangarSupplyCargo(
                  jsonCmd.StarbaseHangarSupplyCargo
                );
                break;
              case jsonCmd.StarbaseHangarUnloadCargo !== undefined:
                ixs = await handlerStarbaseHangarUnloadCargo(
                  jsonCmd.StarbaseHangarUnloadCargo
                );
                break;
              default:
