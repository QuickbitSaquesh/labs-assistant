...
beforeAll(async () => {
  ...
  if (!secretKey) {
    throw new Error(
      "STAR_ATLAS_WALLET_SECRET_KEY environent variable is not set"
    );
  }
...
describe("SAGE Labs (tx)", () => {
  ...
  describe.skip("Fleet Handler - Docking Actions", () => {
    beforeAll(async () => {
      ...
      fleetPubkey = sageGameHandler.getFleetAddress(
        playerProfilePubkey,
        cargoFleetName
      );
  ...
  describe.skip("Fleet Handler - Movement/Warp Actions", () => {
    ...
    test.skip("Warp to Coordinate", async () => {
      ...
      const coordinates: [BN, BN] = [
        sector[0].add(new BN(1)),
        sector[1].add(new BN(1)),
      ];

      expect(`X: ${sector[0]} | Y: ${sector[1]}`).toBe(`X: 40 | Y: 30`);
      expect(`X: ${coordinates[0]} | Y: ${coordinates[1]}`).toBe(
        `X: 41 | Y: 31`
      );
